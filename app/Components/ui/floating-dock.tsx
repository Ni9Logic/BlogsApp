    ...
    <div className={cn("fixed bottom-4 right-4 z-50", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute bottom-full mb-2 right-0 flex flex-col items-center gap-2"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "fixed bottom-8 left-1/2 transform -translate-x-1/2 flex h-20 gap-6 items-end rounded-3xl bg-gray-50 dark:bg-neutral-900 px-6 pb-4 shadow-lg",
        className
      )}
    >
      {items.map((item) => (
